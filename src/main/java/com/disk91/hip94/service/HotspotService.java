...
public class HotspotService {
    ...
    private ObjectCache<String, Hotspot, HotspotService> heliumHotspotCache;
    ...
    private void initHotspotService() {
        this.heliumHotspotCache = new ObjectCache<String, Hotspot, HotspotService>(
            "HotspotCache",
    ...
    public void updateStats(Hotspot h, boolean force) {
        ...
        h.getWitnesses().parallelStream().forEach( w -> {
            try {
                ...
                log.warn("Wit Hotspot "+h.getHotspotId()+" not found");
            }
        });
        ...
        hss.values().parallelStream().forEach( w -> {
            try {
                ...
                log.warn("Wit Hotspot "+h.getHotspotId()+" not found 2");
            }
        });
